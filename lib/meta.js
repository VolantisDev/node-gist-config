...
function build (config, override) {
    ...
    const meta = Object.assign({
        lastSaved: config.get('gistConfig.lastSaved', date),
        lastSynced: config.get('gistConfig.lastSynced', date)
    }, override)
...
function save (config, meta) {
    config.set('gistConfig.lastSaved', meta.lastSaved)
    config.set('gistConfig.lastSynced', meta.lastSynced)
}
