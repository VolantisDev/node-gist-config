/**
 * This is a simple singleton config class. Pass in a config object to 
 * initialize, and then simply call it as an empty function to retrieve the 
 * existing object.
 */

const fs = require('fs')
const appRoot = require('app-root-path')
const Conf = require('../../node-optional-conf/singleton')
 ...
 *   gitHubToken: A token generated by the user. If store is true, it will 
 ...
 * @param {*} userConfig 
 */
module.exports = (cfg) => Conf(mergeConfig(cfg), cfg)

function getProjectName(cfg) {
    var projectName = cfg.projectName
    if (!projectName && fs.existsSync(appRoot + '/package.json')) {
        var packageName = require(appRoot + '/package.json').name
        if (packageName) {
...
function mergeConfig (cfg) {
    var persist = typeof cfg.persist !== 'undefined' ? cfg.persist : true
    var defaults = cfg
    delete defaults.persist

    var config = {
        persist: persist,
        defaults: { gistConfig: Object.assign(defaults, getDefaults) },
        projectName: getProjectName(cfg)
    }
